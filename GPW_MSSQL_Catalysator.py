...
for index,row in imp_df.iterrows():
    cursor.execute('INSERT INTO dbo.[CATALYSATOR]([EMITENT],[KOD],[WART_NOM],[WALUTA],[TYP_OPR],[BAZA_OPR],[CURR_OPR],[LICZBA_OKRESOW],[FREQ_WYPLAT],[LICZBA_POZ_OKRESOW],[NEXT_PAY],[NEXT_PAY_PRAWA],[WYKUP]) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)',
                    row[0], 
                    row[1], 
                    row[2],
                    row[3],
...
pause("Koniec. Naciśnij dowolny klawisz aby kontynuować.")
